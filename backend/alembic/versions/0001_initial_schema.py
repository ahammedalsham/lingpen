...
def upgrade() -> None:
    ...
    op.execute(
        "CREATE TYPE annotation_status AS ENUM ('draft','under_review','approved','rejected')"
    )
    ...
    op.create_index("ix_sentences_metadata_gin", "sentences", ["metadata"], postgresql_using="gin")
    ...
    op.create_table(
        ...
        sa.Column(
            "reverts_revision_id",
            sa.BigInteger(),
            sa.ForeignKey("annotation_revisions.id", ondelete="SET NULL"),
            nullable=True,
        ),
    )
    op.create_index(
        "ix_revisions_token_timestamp", "annotation_revisions", ["token_id", "timestamp"]
    )
    op.create_index(
        "ix_revisions_user_timestamp", "annotation_revisions", ["changed_by", "timestamp"]
    )
    ...
    op.create_index(
        "ix_validation_sentence_severity", "validation_reports", ["sentence_id", "severity"]
    )
