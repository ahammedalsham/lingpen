...
from app.services import delete_user, get_user_by_id, update_user
    ...
    **Note:** Users can only update their own profile
    ...
    try:
        updated_user = await update_user(
            db, user_id, display_name=user_update.display_name, institution=user_update.institution
        )
...
@router.delete("/{user_id}", status_code=status.HTTP_204_NO_CONTENT, summary="Delete user account")
async def delete_user_account(
    ...
    **Note:**
    ...
    if current_user.id != user_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Can only delete your own account"
        )
